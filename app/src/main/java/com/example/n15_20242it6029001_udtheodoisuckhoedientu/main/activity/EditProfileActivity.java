...
public class EditProfileActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.ntv_activity_edit_profile);
    ...
    private void displayProfileInfor(Profile profile){
        if(profile!=null){
            ...
            if("Nam".equals(profile.getSex())){
                radMale.setChecked(true);
            }
            if("Nữ".equals(profile.getSex())){
                radFeMale.setChecked(true);
            }
            String diseases = profile.getDiseases();
            String[] disease = diseases.split(",");
            for(String i : disease){
                if(i.equalsIgnoreCase("Mệt mỏi")){
                    cb1.setChecked(true);
                }
                if(i.equalsIgnoreCase("Huyết áp cao")){
                    cb2.setChecked(true);
                }
                if(i.equalsIgnoreCase("Tiểu đường")){
                    cb3.setChecked(true);
                }
                if(i.equalsIgnoreCase("Rối loạn tâm thần")){
                    cb4.setChecked(true);
                }
                if(i.equalsIgnoreCase("Không có bệnh nền")){
    ...
    private void updateProfile(Profile profile){
        ...
        if(update){
            Toast.makeText(getApplicationContext(), "Sửa hồ sơ thành công", Toast.LENGTH_SHORT).show();
        }else{
            Toast.makeText(getApplicationContext(), "Lỗi khi update hồ sơ", Toast.LENGTH_SHORT).show();
        }
    }
